...
Uses hand-written fake chromadb / openai / huggingface_hub / datasets / ragas
...
def check(name, condition, detail=""):
    """Check a condition and record pass/fail."""
    if condition:
        PASS.append(name)
        print(f"  [PASS] {name}")
    else:
...
check("5 source docs loaded", manifest1.source_doc_count == 5,
      f"got {manifest1.source_doc_count}")
check("chunks were produced", manifest1.chunk_count > 0,
      f"got {manifest1.chunk_count}")
check(
    "avg chunk size is reasonable (<=900 chars)",
    manifest1.avg_chunk_chars <= 900,
    f"got {manifest1.avg_chunk_chars}"
)
check("embedding model recorded",
      manifest1.embedding_model == "sentence-transformers/all-MiniLM-L6-v2")
check(
    "all 5 doc hashes recorded",
    len(manifest1.source_doc_hashes) == 5,
    f"got {list(manifest1.source_doc_hashes)}"
)
...
check("staging collection exists after build",
      STAGING_COLLECTION in staging_names, f"got {staging_names}")
...
check(
    "hard-block guardrail correctly fails this manifest (contains an AWS key)",
    not r_pii_block.passed,
    r_pii_block.detail
)

clean_manifest = copy.deepcopy(manifest1)
clean_manifest.has_hard_block = False
r_pii_clean = check_pii_hard_block(clean_manifest)
check("hard-block guardrail passes when has_hard_block is False",
      r_pii_clean.passed, r_pii_clean.detail)

print("\n=== 2c. answer-groundedness guardrail (check_answer_guardrails) ===")
grounded_verdicts = [{"question": "q1", "verdict": {"grounded": True, "reason": "ok"}}]
ungrounded_verdicts = [
    {"question": "q1", "verdict": {"grounded": True, "reason": "ok"}},
...
check("answer guardrail passes when all verdicts are grounded",
      r_grounded.passed, r_grounded.detail)
check("answer guardrail fails when any verdict is ungrounded",
      not r_ungrounded.passed, r_ungrounded.detail)
...
check("drifted embedding model is correctly rejected", not r_drift.passed,
      r_drift.detail)
...
check(f"missing doc ({missing_doc}) is correctly rejected",
...
check(
    "retrieval quality gate returns all 3 metrics",
    set(r_quality.metrics.keys()) == {"faithfulness", "context_precision",
                                       "context_recall"},
    r_quality.metrics
)
print(f"  (fake-judge scores -- not meaningful as real quality signal: "
      f"{r_quality.metrics})")
...
check("prod collection exists after promotion",
      PROD_COLLECTION in prod_names, f"got {prod_names}")

hits = retrieve("What caused the checkout latency spike?",
                collection_name=PROD_COLLECTION, k=3)
check("prod retrieval returns hits", len(hits) > 0, f"got {len(hits)}")
check("retrieved hits carry doc_id metadata",
      all("doc_id" in h["metadata"] for h in hits))

print("\n=== 9. include.query CLI path (ask()) ===")
captured = {}
...
try:
    ...
    check(
        ...
        captured.get("cmd", [])[:4] == ["airflow", "dags", "trigger",
                                         "postmortem_query_pipeline"],
        captured.get("cmd"),
    )
    check(
...
with open(fake_eval_path, "w", encoding="utf-8") as f:
    f.write(
        json.dumps(
            {"question": "Why did customers get charged twice?",
             "ground_truth": "existing ground truth"}
        ) + "\n"
    )

conn = sqlite3.connect(fake_db_path)
conn.execute(
    "CREATE TABLE query_results (run_id TEXT PRIMARY KEY, status TEXT NOT NULL, "
    "question TEXT, text TEXT NOT NULL, sources_json TEXT, ts TEXT NOT NULL)"
)
fake_rows = [
    ...
    (
        ...
        "answered",
        "What caused the checkout latency spike?  ",
        "Connection pool exhaustion, latest.",
        "2026-01-03T00:00:00Z"
    ),
    # asked 3x but already in the golden set -> should be skipped
    (
        "run-b1",
        "answered",
    ...
    # only asked twice -> below min_asked_count=3 threshold
    (
        "run-c1",
        "answered",
        "What's the on-call escalation policy?",
        "Some answer.",
        "2026-01-01T00:00:00Z"
    ),
    (
    ...
    # asked 3x but never successfully answered
    (
        "run-d1",
        "refused",
...
for run_id, status, question, text, ts in fake_rows:
    conn.execute(
        "INSERT INTO query_results (run_id, status, question, text, sources_json, ts) "
        "VALUES (?,?,?,?,?,?)",
...
result = add_frequent_questions_to_golden_set(
    fake_eval_path, min_asked_count=3, query_results_db_path=fake_db_path)
with open(fake_eval_path, encoding="utf-8") as f:
    golden_rows_after = [json.loads(line) for line in f if line.strip()]
...
check(
    ...
    any(
        r["question"] == "What caused the checkout latency spike?"
        and r["ground_truth"] == "Connection pool exhaustion, latest."
...
result2 = add_frequent_questions_to_golden_set(
    fake_eval_path, min_asked_count=3, query_results_db_path=fake_db_path)
check("re-running grow_golden_set on the same data adds nothing new",
      result2["added"] == [], result2)
...
result3 = add_frequent_questions_to_golden_set(
    fake_eval_path, min_asked_count=3, query_results_db_path=missing_db_path)
check(
    "missing query_results.db is handled as a harmless no-op",
    result3 == {"added": [], "candidates_seen": 0},
    result3
)
